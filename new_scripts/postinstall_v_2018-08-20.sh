...
for i in "$@"; do
  case $i in
	--help|-h)
	  echo ""
	  echo "In general, run this script with no arguments on a fresh CentOS v7.x"
	  echo "minimal install to set up a Lab Server that can deploy Lab Workstations."
	  echo ""
	  ...
	  echo "  --noconfig		 Do not run any additional configuration steps"
	  echo "  --noldapconfig	 Do not run the LDAP configuration steps"
	  echo "  --nokerberosconfig Do not run the Kerberos configuration steps"
...
if [ 2 -lt ${NUM_OF_NICS} ]; then
  ...
  echo "Next I'll try to figure out which NIC connects to the Internet.  The other NIC we'll use for a PRIVATE network where you can PXE boot your student workstation(s)." | tee -a "${LOG}"
  findExternalNIC
fi
}
...
findExternalNIC(){
GW1="${PITD}/GW"
ip route | grep 'default' > ${GW1}
if [ 1 -eq $(wc -l ${GW1}) ]; then
  GW1_IP=$(cut -d' ' -f3 ${GW1})
  GW1_DEV=$(cut -d' ' -f5 ${GW1})
  echo "Default gateway found as ${GW1_IP} using ${GW1_DEV}." | tee -a "${LOG}"  
elif [ 1 -lt $(wc -l ${GW1}) ]; then
  echo "More than one default gateway?  That probably won't work." | tee -a "${LOG}"  
  exit 3
  ## Not sure why or how this would happen, but give it exit code 3
else
  echo "No default gateway found." | tee -a "${LOG}"
  echo "Fix your networking and try again." | tee -a "${LOG}"
fi  
}

getNICOrder(){

#### The old way is below, but if we find the external NIC as above
#### the other NIC _must_ be used to PXE boot workstations, right?
################################################################## 
...
NIC1NAME=${GW1_DEV}
NIC1CON=$(nmcli device show ${NIC1NAME} | grep 'GENERAL.CONNECTION' | tr -s ' ' | cut -d' ' -f2)
NIC1CONUUID=$(nmcli con show ${NIC1CON} | grep 'connection.uuid' | tr -s ' ' | cut -d' ' -f2)

NIC2NAME=$(grep -v ${GW1_DEV} ${PITD}/NICs | head -n 1 )
NIC2CON=$(nmcli device show ${NIC2NAME} | grep 'GENERAL.CONNECTION' | tr -s ' ' | cut -d' ' -f2)
NIC2CONUUID=$(nmcli con show ${NIC2CON} | grep 'connection.uuid' | tr -s ' ' | cut -d' ' -f2)
...
case ${DETECTEDOS} in
  ...
  12) # CentOS v7.2
	...
	ISOURL[2]="http://isoredirect.centos.org/centos/7/isos/x86_64/CentOS-7-x86_64-DVD-1511.iso"
	ISOURL[2]="http://mirror.vtti.vt.edu/centos/7.2.1511/isos/x86_64/CentOS-7-x86_64-DVD-1511.iso"
    ISOSIZE=4329570304
